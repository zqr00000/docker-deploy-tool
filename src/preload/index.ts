...
const electronAPI: ElectronAPI = {
  ...
  fileTransfer: {
    ...
    localOp: (op: 'mkdir' | 'touch' | 'rename' | 'delete', target: string, to?: string): Promise<{ success: boolean; message?: string }> => ipcRenderer.invoke('fileTrans:localOp', op, target, to),
    remoteOp: (serverId: string, op: 'mkdir' | 'touch' | 'rename' | 'delete', target: string, to?: string): Promise<{ success: boolean; message?: string }> => ipcRenderer.invoke('fileTrans:remoteOp', serverId, op, target, to),
  ...
  opsAgent: {
    ...
    onDone: (callback: (payload: { requestId: string; truncated?: boolean }) => void): (() => void) => {
      const listener = (_e: any, payload: { requestId: string; truncated?: boolean }) => callback(payload)
